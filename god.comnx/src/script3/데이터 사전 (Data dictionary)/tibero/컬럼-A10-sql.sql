...
SELECT
     ...
     , '     , ' || '''''' || ' /* ' || COLUMN_COMMENTS || ' */' AS SQL_INSERT_VALUES
--
     , '     , A.' || COLUMN_NAME || ' /* ' || COLUMN_COMMENTS || ' */' AS SQL_SELECT
     , '   AND A.' || COLUMN_NAME || ' = '''' /* ' || COLUMN_COMMENTS || ' */' AS SQL_SELECT_WHERE
--
     , '     , ' || COLUMN_NAME || ' = '''' /* ' || COLUMN_COMMENTS || ' */' AS SQL_UPDATE_SET
     , '   AND ' || COLUMN_NAME || ' = '''' /* ' || COLUMN_COMMENTS || ' */' AS SQL_UPDATE_WHERE
--
     , '   AND ' || COLUMN_NAME || ' = '''' /* ' || COLUMN_COMMENTS || ' */' AS SQL_DELETE_WHERE
--
     , A.*
  FROM A
;
